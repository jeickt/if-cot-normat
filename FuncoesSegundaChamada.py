                ...
                if ws[f'A{count}'].value == None:
                    ...
                    if j == 1:
                        ...
                        if list(filter(lambda x: x.codigo == ws[f'A{count}'].value, listasPrimeiraChamada[i])):
                            cand = list(filter(lambda x: x.codigo == ws[f'A{count}'].value, listasPrimeiraChamada[i]))[0]
            ...
            for cand in curso[sequencia[i]]:
                if cand.valido == "SIM":
                    ...
                    if not possivelChamado.matricula and not possivelChamado.ausencia \
                            and possivelChamado.chamada in [0, 1]:
...
def fazerArquivosDeChamada():
    for i in range(len(segundasChamadas)):
        ...
        df.to_excel(f'C:\\temp-SegundaChamada\\SC-{listasCursos[i][0].nome[0]}-{listasCursos[i][0].nome[1]}.xlsx', index=False)
