        ...
        for j in range(1, 12, 1):
            ...
            while verificador:
                if ws[f'A{count}'].value == None:
                    ...
                    if j == 1:
                        listaCota.append(CandControle(ws[f'A{count}'].value, ws[f'B{count}'].value, ws[f'C{count}'].value,
                                                     ws[f'D{count}'].value, ws[f'E{count}'].value, ws[f'F{count}'].value,
                                                     ws[f'G{count}'].value, ws[f'H{count}'].value, ws[f'I{count}'].value,
                                                     ws[f'J{count}'].value, ws[f'K{count}'].value, ws[f'L{count}'].value))
            ...
            if cand.descPPI:
                for j in [2, 3, 6, 7, 11]:
                    if list(filter(lambda x: x.codigo == cand.codigo, listasCursos[i][j])):
                        ...
                        ws = wb.get_sheet_by_name(f'Cota-{j}')
                        ws[f'F{listasCursos[i][j].index(candResumido) + 2}'] = "NAO"
            if cand.descRI:
                for j in [2, 3, 4, 5]:
                    if list(filter(lambda x: x.codigo == cand.codigo, listasCursos[i][j])):
                        candResumido = list(filter(lambda x: x.codigo == cand.codigo, listasCursos[i][j]))[0]
        ...
        if curso[12][i][1] != 0:
            for cand in curso[sequencia[i]]:
                if cand.valido == "SIM":
                    ...
                    if not possivelChamado.matricula and not possivelChamado.ausencia \
                            and possivelChamado.chamada in [0, 1, 2]:
...
def fazerArquivosDeChamada():
    for i in range(len(terceirasChamadas)):
        ...
        for c in range(len(terceirasChamadas[i])):
            data.append([terceirasChamadas[i][c].codigo, terceirasChamadas[i][c].nome, terceirasChamadas[i][c].inscricao,
                         terceirasChamadas[i][c].tipoVaga, terceirasChamadas[i][c].chamada])
        df = pd.DataFrame(data, columns=['Código', 'Nome', 'Inscrição', 'Tipo da Vaga', 'Chamada'])
        df.to_excel(f'C:\\temp-TerceiraChamada\\TC-{listasCursos[i][0].nome[0]}-{listasCursos[i][0].nome[1]}.xlsx', index=False)
